...
def get_activations(files, model, batch_size=50, dims=2048, cuda=False):
    ...
    if torch.is_tensor(files):
        ...
        images = files # .astype(np.float32)
        ...
        for i in tqdm(range(0, imagesBig.shape[0], batch_size)):
            start = i
            end = i + batch_size

            batch = imagesBig[start:end,:,:,:]
            if cuda:
                batch = batch.cuda()
...
def getRealm1s1(paths, batch_size, cuda, dims,model):
    m1, s1 = _compute_statistics_of_path(paths[0], model, batch_size, dims, cuda)
    return m1, s1

def calculate_fid_given_paths(paths, batch_size, cuda, dims,model= None,m1=None, s1=None):
